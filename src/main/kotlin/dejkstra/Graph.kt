...
class MyGraph<T, E : Number>(val directed: Boolean = false, val defaultCost: E): Graph<T, E> {
     val edges = mutableSetOf<Edge<T, E>>()
    private val vertices = mutableSetOf<T>()

    fun addVertex(node: T): MyGraph<T, E> {
        vertices += node
        return this
    }

    fun addArc(pair: Pair<T, T>, value: E? = null): MyGraph<T, E> {
        val (from, to) = pair
        addVertex(from)
        addVertex(to)
